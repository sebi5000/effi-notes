...
| **7** | Dokumentation, CI, Lifecycle: docs/architecture.md (C4 L1+L2 Mermaid), docs/operations.md (Vendor-Runbook), docs/customer-install.md (Kunde-Admin), GitHub-Actions CI + Image-Build + Trivy-Scan + Release, Changesets-Setup, next-intl DE/EN-Setup mit Locale-Resolver, fünf Subagents in `.claude/agents/` | Docs lesbar und akkurat, CI-Workflows valide, Trivy-Scan mit `exit-code: 0` als Baseline, Subagents von Claude Code aufrufbar, i18n in Login/Dashboard/Home verdrahtet |
| **8** | Validierung: Vitest-Smoke-Tests (env-Schema, RBAC-Helper), `deploy/scripts/smoke.sh` (frischer Clone → `make smoke` baut, fährt hoch, verifiziert Endpoints + Backup-Roundtrip), `compose.smoke.yml` Override (Caddy aus, Ports exposed), README + Spec final | Vitest grün, smoke.sh erfolgreich auf macOS lokal verifiziert |
