...
describe('socket lifecycle', () => {
  ...
  it('relays updates to other peers and stores them in the in-memory doc', async () => {
    ...
    const closeA = await onSocketOpen({ noteId: note.id, userId: author.id, socket: a });
    const closeB = await onSocketOpen({ noteId: note.id, userId: author.id, socket: b });
    ...
    await handleMessage({ noteId: note.id, userId: author.id, socket: a }, encodeUpdate(update));
  ...
  it('responds to sync-step-1 with step-2 + step-1', async () => {
    ...
    const sock = new FakeSocket();
    const close = await onSocketOpen({ noteId: note.id, userId: author.id, socket: sock });
    sock.sent = [];
    ...
    await handleMessage(
      { noteId: note.id, userId: author.id, socket: sock },
    ...
    await handleMessage({ noteId: note.id, userId: author.id, socket: sock }, encodeSyncStep1(sv));
